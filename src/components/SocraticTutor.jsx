...
import CoachKnowledgeCheck from './CoachKnowledgeCheck';
...
export default function SocraticTutor({
  ...
  return (
    <div className="space-y-4">
      {!hasCompletedCheck && (
        <CoachKnowledgeCheck currentConcept={currentConcept} onPassed={() => setHasCompletedCheck(true)} />
      )}
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 relative z-10">
              <div>
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  {/* Ollama Status Pill */}
                  <div className={`inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-full text-2xs font-mono border ${isOnline
              ...
              {TUTOR_MODES.map((mode) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-2xs whitespace-nowrap transition-all cursor-pointer ${isActive
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                  <div className={`max-w-[85%] rounded-2xl p-4 text-xs leading-relaxed shadow-md ${msg.sender === 'user'
